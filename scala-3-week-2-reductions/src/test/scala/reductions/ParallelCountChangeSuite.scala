...
class SequentialCountChangeSuite extends FunSuite {
...
@RunWith(classOf[JUnitRunner])
class ParallelWithNoThresholdCountChangeSuite extends FunSuite {

  test("parCountChange should return 0 for money < 0") {
    def check(money: Int, coins: List[Int]) =
      assert(parCountChange(money, coins, noThreshold) == 0,
  ...
  test("parCountChange should return 1 when money == 0") {
    def check(coins: List[Int]) =
      assert(parCountChange(0, coins, noThreshold) == 1,
  ...
  test("parCountChange should return 0 for money > 0 and coins = List()") {
    def check(money: Int) =
      assert(parCountChange(money, List(), noThreshold) == 0,
...
@RunWith(classOf[JUnitRunner])
class ParallelWithMoneyThresholdCountChangeSuite extends FunSuite {
...
@RunWith(classOf[JUnitRunner])
class ParallelWithCoinsThresholdCountChangeSuite extends FunSuite {
...
@RunWith(classOf[JUnitRunner])
class ParallelWithCombinedThresholdCountChangeSuite extends FunSuite {
