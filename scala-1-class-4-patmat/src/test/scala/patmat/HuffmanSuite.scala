...
class HuffmanSuite extends FunSuite {
  ...
  test("times") {
    new TestLists {
      assert(times(empty) === List())
      assert(times(abaac) === List(('a', 3), ('b', 1), ('c', 1)))
    }
  }
  ...
  test("max") {
    new TestLists {
      assert(_max(List(('a', 3), ('b', 1), ('c', 1))) === ('a', 3))
    }
  }

  test("sort") {
    new TestLists {
      assert(_sort(unsorted) === List(('c', 1), ('d', 2), ('a', 3), ('b', 7)))
    }
  }
  ...
  test("until singleton combine trees") {
    new TestTrees {
      assert(until(singleton, combine)(List(t1, t2, t3)).length === 1)
    }
  }

  test("decode french code") {
    new TestFrenchCodeAndSecret {
  ...
  test("decode and encode a text using frenchCode should be identity") {
    new TestFrenchCodeAndSecret {
      assert(decode(frenchCode, encode(frenchCode)("jaimelefromage".toList)) === "jaimelefromage".toList)

      assert(decode(frenchCode, quickEncode(frenchCode)("jaimelefromage".toList)) === "jaimelefromage".toList)
    }
  }
