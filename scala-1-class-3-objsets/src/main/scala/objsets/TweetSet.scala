...
case class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  override def filter(p: Tweet => Boolean): TweetSet =
    if (!p(elem)) left.filter(p) union right.filter(p)
    else NonEmpty(elem, left.filter(p), right.filter(p))

  def filterAcc(p: Tweet => Boolean, acc: TweetSet): TweetSet = ???

  override def mostRetweeted: Tweet = max(this)

  def max(x: Int, y: Int): Int = if (x > y) x else y

  def max(x: Tweet, y: Tweet): Tweet = if (x.retweets > y.retweets) x else y

  def max(s: TweetSet): Tweet = s match {
    case NonEmpty(e, Empty(), Empty()) => e
    case NonEmpty(e, Empty(), r) => max(e, max(r))
    case NonEmpty(e, l, Empty()) => max(e, max(l))
    case NonEmpty(e, l, r) => max(e, max(max(l), max(r)))
    case _ => throw new IllegalArgumentException("max of empty")
  }
  ...
  override def union(that: TweetSet): TweetSet = that match {
    case NonEmpty(el, l, r) => ((this incl el) union l) union r
    case _ => this
  }
...
object GoogleVsApple {
  ...
  lazy val googleTweets: TweetSet = TweetReader.allTweets
    .filter(t => google.exists(p => t.text.contains(p)))
  lazy val appleTweets: TweetSet = TweetReader.allTweets
    .filter(t => apple.exists(p => t.text.contains(p)))
